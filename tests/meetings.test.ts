...
vi.mock("../src/server/services/openai.js", () => ({
  ...
  generateSummaryAndKeyPoints: vi.fn().mockResolvedValue({
    summary:
      "This meeting covered quarterly planning with key stakeholders.",
    keyPoints: [
...
vi.mock("../src/server/services/s3.js", () => ({
  getSignedUrlForKey: vi
    .fn()
    .mockResolvedValue(
      "https://test-bucket.s3.amazonaws.com/audio/test.webm?signed=true"
    ),
  uploadAudioToS3: vi.fn().mockResolvedValue({
    audioUrl: "https://test-bucket.s3.amazonaws.com/audio/test.webm?signed=true",
...
// Mock DB using vi.fn() only (no top-level variable references in factory)
...
describe("Meetings Router", () => {
  describe("meetings.create", () => {
    beforeEach(async () => {
      ...
      // Each create test: insert returns id 1, then findFirst returns the created meeting
      (db.insert as ReturnType<typeof vi.fn>).mockReturnValue({
        values: vi.fn().mockReturnValue({
          $returningId: vi.fn().mockResolvedValue([{ id: 1 }]),
        }),
      });
      (db.query.meetings.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(pendingMeeting);
    });
    ...
    it("should create a meeting with required fields", async () => {
      ...
      const result = await caller.create({
        ...
        audioUrl: "https://test-bucket.s3.amazonaws.com/audio/test.webm",
        audioKey: "audio/test_123.webm",
      });
    ...
    it("should reject meeting with invalid audioUrl", async () => {
      const caller = createCaller();
      await expect(
        caller.create({
          title: "Test Meeting",
  ...
  describe("meetings.getById", () => {
    it("should retrieve a meeting by ID", async () => {
      const { db } = await import("../src/server/db/index.js");
      (db.query.meetings.findFirst as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
        pendingMeeting
      );
  ...
  describe("meetings.list", () => {
    beforeEach(async () => {
      const { db } = await import("../src/server/db/index.js");
      (db.query.meetings.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(
        [pendingMeeting, doneMeeting]
      );
    });
  ...
  describe("meetings.processAudio", () => {
    ...
    it("should throw NOT_FOUND if meeting doesn't exist", async () => {
      ...
      const caller = createCaller();
      await expect(
        caller.processAudio({ meetingId: 9999 })
